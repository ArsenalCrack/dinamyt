 ...
 * (`OPERAR.md`, §4.9). Hasta aquí el portal era el único de los
 * tres que seguía con `<input type="date">`.
 *
 * ── Por qué no `<input type="date">` ──
 *
 * Porque en Android el calendario nativo solo se mueve MES A MES. Para poner
 * una fecha de nacimiento hay que pulsar la flecha trescientas veces, y el año
 * ni siquiera se ve hasta cruzar diciembre. En el registro del portal, donde la
 * fecha que se pide es justo una de nacimiento, eso no es un detalle de estilo:
 * es la razón por la que el campo se queda vacío.
 *
 ...
 * días. Tres toques para cualquier fecha de cualquier década.
 *
...
/** Hoy, en horario local (no UTC: `toISOString` cambia el día por la tarde). */
function hoyISO(): string {
  const d = new Date();
  return iso(d.getFullYear(), d.getMonth() + 1, d.getDate());
}

/** La fecha escrita como la lee una persona. */
...
/** Nombres cortos de los días, sin diccionario propio. */
function diasCortos(): string[] {
  const fmt = new Intl.DateTimeFormat('es-CO', { weekday: 'narrow' });
...
function mesesDelAno(largo: boolean): string[] {
  const fmt = new Intl.DateTimeFormat('es-CO', { month: largo ? 'long' : 'short' });
...
export function CampoFecha({
   ...
   * solo admite el pasado, como el nacimiento), en el tope más cercano.
   */
  ...
  const meses = useMemo(() => mesesDelAno(false), []);
  const mesesLargos = useMemo(() => mesesDelAno(true), []);
  const semana = useMemo(() => diasCortos(), []);
  ...
  // Cerrar al tocar fuera, con Escape o si la página se mueve. `touchstart`
  // además de `mousedown` porque en el celular no hay `mousedown`; y la tecla
  // porque sin ella el panel atrapa al teclado.
  useEffect(() => {
    if (!abierto) return;
  ...
  function abrir() {
    ...
    // Sin fecha puesta se abre DIRECTO en los años: quien va a escribir su
    // nacimiento no quiere ver el mes que viene.
    setVista(elegida ? 'dias' : 'anos');
    setAbierto(true);
  }
  ...
  const etiqueta = valor ? fmtFecha(valor) : (placeholder ?? 'Elegir fecha');
  ...
  return (
    <div ref={raizRef} className="campofecha" style={style} onKeyDown={alTeclear}>
      ...
      >
        <span className="fecha-btn-label" data-vacio={!valor ? true : undefined}>
          {etiqueta}
        </span>
      ...
      {abierto &&
        ...
        createPortal(
          <div
            ...
            role="dialog"
            aria-label={etiquetaAria ?? 'Elegir fecha'}
            style={{
              left: caja.left,
              width: caja.width,
          ...
          >
            {/* ── Cabecera: el título ES el botón que cambia de vista ──
                ...
                problema de fondo: llegar a 1998 sin cruzar trescientos meses. */}
            <div className="fecha-cabecera">
              ...
              >
                ...
                <span className="fecha-titulo-flecha" aria-hidden="true">
                  ▾
                </span>
              </button>
              <button
