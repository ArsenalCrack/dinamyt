/**
 * Catálogo geográfico (país → ciudades) para los desplegables del portal.
 *
 * Es el MISMO catálogo que usa Campeonatos (`frontend/src/lib/geo.ts` y su
 * espejo `backend/app/geo.py`), copiado aquí a propósito. Antes el portal lo
 * pedía por HTTP a `campeonatos-api` (`GET /geo/paises`), una ruta que no
 * existe: la llamada fallaba siempre, el `catch` dejaba la lista vacía y los
 * desplegables de país y ciudad de «Fundar mi club» se quedaban con Colombia
 * como única opción y sin una sola ciudad. Un catálogo de país y ciudad no
 * cambia de un día para otro y no vale una dependencia entre servicios.
 *
 * Se guarda el NOMBRE, no el ISO: es lo que ya hay escrito en
 * `organizations.country` / `.city`, y lo que Campeonatos compara por valor
 * exacto para agrupar sus reportes.
 */
