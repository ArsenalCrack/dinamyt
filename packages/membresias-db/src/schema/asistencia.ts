...
// ── Plantillas biométricas (dato sensible — cifrado, con consentimiento) ─────
export const biometricTemplates = mem.table('biometric_templates', {
  id: uuid('id').primaryKey().defaultRandom(),
  membershipId: uuid('membership_id')
    .notNull()
    .references(() => memberships.id),
  /** Plantilla cifrada (base64). NUNCA la imagen de la huella. */
  template: text('template').notNull(),
  /** Marca/formato de la plantilla (vendor lock-in): p. ej. 'digitalpersona'. */
