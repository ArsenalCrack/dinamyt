...
/**
 * Conexión a la BD propia de Academy.
 * - Producción / Supabase: cliente `postgres` (postgres-js) vía `ACADEMY_DATABASE_URL`.
 * - Desarrollo local sin servidor: si `ACADEMY_PGLITE_DATA` está definida, se usa
...
function crearDb(): PostgresJsDatabase<typeof schema> {
  if (process.env.ACADEMY_PGLITE_DATA) {
    ...
    const client = new PGlite(process.env.ACADEMY_PGLITE_DATA);
    return pgliteDrizzle(client, {
      schema,
    }) as unknown as PostgresJsDatabase<typeof schema>;
  }

  const client = postgres(process.env.ACADEMY_DATABASE_URL!, {
