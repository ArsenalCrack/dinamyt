 ...
 * ── Por qué existe ──
 *
 * Cada pantalla avisaba del resultado de una acción con una franja ARRIBA DEL
 * TODO. Funciona mientras quepa la página entera en el alto de la pantalla, y
 * casi nunca cabe: al guardar un competidor desde el final de un formulario
 * largo, o al desactivar a un juez desde el fondo de la lista, el mensaje
 * aparecía a cientos de píxeles por encima —fuera de la vista— y se borraba
 * solo a los tres segundos. Desde donde estaba el usuario no pasaba nada: ni
 * "guardado" ni "no se pudo". Volvía a pulsar, y ahí sí se creaban dos.
 *
 ...
 *     await guardarAPI(datos);
 *     await recargar();
 *     avisoOk(t("admin.usuarios.creado"));
 *
