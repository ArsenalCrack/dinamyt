 ...
 * «entrena desde 2016» hay que pulsar la flecha ciento y pico veces, y el año
 * ni siquiera se ve hasta que se cruza diciembre. En un formulario donde la
 * fecha que más se escribe es una vieja —desde cuándo entrena el alumno que
 * lleva media vida en el club— eso no es un detalle de estilo: es la razón por
 * la que el campo se queda vacío.
 *
 ...
 * iPhone y en el portátil, y con el mismo panel dorado que los desplegables de
 * la app (`<SelectMenu>`) en vez de la caja gris del sistema operativo.
 *
 * El valor que entra y sale es siempre 'YYYY-MM-DD' —lo mismo que daba el
 * input nativo—, así que las pantallas que lo usan no notan el cambio.
 */
...
export function CampoFecha({
   ...
   * solo admite el pasado lejano), en el tope más cercano.
   */
  ...
  const [primerAno, setPrimerAno] = useState(
    () => cursor.a - (cursor.a % AÑOS_POR_PAGINA),
  );
  ...
  /**
   * El panel se dibuja en el `<body>`, con las coordenadas del botón. Mismo
   * motivo que en `<SelectMenu>`: dentro de una tarjeta con `overflow` un
   * panel hijo sale recortado. Si abajo no cabe, se abre hacia arriba.
   */
  ...
  // Cerrar al tocar fuera, con Escape o si la página se mueve. Igual que el
  // desplegable: en el celular no hay `mousedown`, y sin la tecla el panel
  // atrapa al teclado.
  useEffect(() => {
    if (!abierto) return;
    ...
    const c =
      dentro < iso(limiteMin.a, limiteMin.m, 1)
        ...
        : dentro > iso(limiteMax.a, limiteMax.m, 1)
          ...
          : { a: base.a, m: base.m };
    setCursor(c);
    setPrimerAno(c.a - (c.a % AÑOS_POR_PAGINA));
    setVista('dias');
    setAbierto(true);
  }
  ...
  return (
    <div ref={raizRef} className="campofecha" style={style} onKeyDown={alTeclear}>
      ...
      >
        ...
        <span className="selectmenu-flecha" aria-hidden="true">
          📅
        </span>
      </button>
        ...
        createPortal(
          ...
          >
            {/* ── Cabecera: el título ES el botón que cambia de vista ──
                ...
                problema de fondo: llegar a 2016 sin cruzar noventa meses. */}
            <div className="fecha-cabecera">
                ...
                onClick={() =>
                  vista === 'dias'
                    ...
                    : vista === 'meses'
                      ...
                      : setPrimerAno((y) => y - AÑOS_POR_PAGINA)
                }
              >
                ‹
              </button>
              ...
              >
                {vista === 'dias'
                  ...
                  : vista === 'meses'
                    ? String(cursor.a)
                    : `${primerAno} – ${primerAno + AÑOS_POR_PAGINA - 1}`}
            ...
            {vista === 'anos' && (
              <div className="fecha-rejilla fecha-rejilla-anos">
                {Array.from({ length: AÑOS_POR_PAGINA }, (_, i) => primerAno + i).map((a) => (
                  <button
                    key={a}
                    type="button"
                    className="fecha-celda"
